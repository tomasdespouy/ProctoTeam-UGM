...
export function ExamFooter() {
  // Solo mide la disponibilidad de la red local (no la conexión al servidor)
  ...
  return (
    <footer 
      // Fondo neutro para contraste, manteniendo la posición fija
      ...
      <div className="flex items-center gap-2 text-xs font-semibold">
        {isOnline ? (
          <div className="flex items-center gap-2 text-green-600 dark:text-green-400">
            ...
            {/* CORRECCIÓN UX: Refleja la verdad técnica */}
            <span>Red Detectada</span>
          </div>
        ) : (
          <div className="flex items-center gap-2 text-red-600 dark:text-red-400">
            <WifiOff className="h-4 w-4" />
            <span>Red Desconectada</span>
          </div>
        )}
      </div>
    </footer>
